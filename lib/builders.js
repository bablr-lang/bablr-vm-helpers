...
const buildLiteralTag = (lit) => freeze({ type: 'LiteralTag', value: lit });
...
export const buildString = (value) => {
  ...
  if (pieces.length === 1 && pieces[0] === "'") {
    return t.node(l.CSTML, 'String', [t.ref`openToken`, t.ref`content`, t.ref`closeToken`], {
      openToken: t.s_node(l.CSTML, 'Punctuator', '"'),
      content: interpolateString(freeze(buildLiteralTag(value))),
      closeToken: t.s_node(l.CSTML, 'Punctuator', '"'),
    });
  }
    ...
    if (isString(piece)) {
      ...
      for (const chr of value) {
        if (
          ...
          if (lit) {
            terminals.push(buildLiteralTag(lit));
            lit = '';
          }
  ...
  if (lit) terminals.push(buildLiteralTag(lit));
  lit = '';
