...
export const buildNodeOpenTag = (flags, language, type, attributes = {}) => {
  ...
  return t.node(
    ...
    [
      ...
      ...when(language, [t.ref`language`, t.ref`languageSeparator`]),
      t.ref`type`,
    ...
    ],
    {
      ...
      language:
        language &&
        (language.startsWith('https://') ? buildString(language) : buildIdentifier(language)),
...
export const buildLiteral = (value) => {
  return t.node(l.CSTML, 'Literal', [t.ref`value`], { value: buildString(value) });
};
...
export const buildString = (value) => {
  ...
  if (value === "'") {
    return i.Expression`"${freeze({ type: 'Literal', value })}"`;
  }
