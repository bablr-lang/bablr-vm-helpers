...
export const buildProperty = (key, value) => {
  return t.node('Instruction', 'Property', [t.ref`key`, t.ref`mapOperator`, buildSpace(), t.ref`value`], {
...
export const buildArray = elements => {
  return _t.node("Instruction", "Array", [_t.ref`open`, ..._interpolateArrayChildren(elements, _t.ref`elements[]`, t.t_node('Comment', null, [t.t_node('Space', 'Space', [t.lit(' ')])])), _t.ref`close`], {
...
export const buildTuple = values => {
  return _t.node("Instruction", "Tuple", [_t.ref`open`, ..._interpolateArrayChildren(values, _t.ref`values[]`, t.t_node('Comment', null, [t.t_node('Space', 'Space', [t.lit(' ')])])), _t.ref`close`], {
    open: _t.s_node("Instruction", "Punctuator", "("),
...
export const buildObject = properties => {
  return _t.node("Instruction", "Object", [_t.ref`open`, ..._interpolateArrayChildren(Object.entries(properties).map(([key, value]) => buildProperty(key, value)), _t.ref`properties[]`, t.t_node('Comment', null, [t.t_node('Space', 'Space', [t.lit(' ')])])), _t.ref`close`], {
    open: _t.s_node("Instruction", "Punctuator", "{"),
...
export const buildAttributes = attributes => {
  return Object.entries(attributes).flatMap(([key, value]) => [buildAttribute(key, value), buildSpace]).slice(0, -1);
};
