...
export const buildNodeOpenTag = (flags, type, attributes = {}) => {
  ...
  return t.node(l.CSTML, 'OpenNodeTag', [t.ref`open`, ...when(flags_, [t.ref`flags`]), t.ref`type`, ...when(attributes_.length, [t.embedded(buildSpace())]), ...interpolateArrayChildren(attributes, t.ref`attributes[]`, t.embedded(buildSpace())), t.ref`close`], {
...
export const buildNodeCloseTag = type => {
  return t.node(l.CSTML, 'CloseNodeTag', [t.ref`open`, ...when(type, [t.ref`type`]), t.ref`close`], {
    open: t.s_node(l.CSTML, 'Punctuator', '</'),
...
export const buildLiteral = value => {
  return _t.node(_l.CSTML, "Literal", [_t.ref`value`], {
