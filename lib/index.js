...
const reifyNodeFlags = (flags) => {
  let { syntactic, escape, trivia } = flags?.properties || {};

  return { syntactic: !!syntactic, escape: !!escape, trivia: !!trivia };
};
...
export const reifyExpression = (node) => {
  ...
  if (!['Instruction', 'String', 'Spamex'].includes(node.language)) {
    return node;
  }

  switch (node.type) {
    case 'NodeMatcher':
      let { flags, type, attributes, value } = node.properties;
      ...
      const children = flags.syntactic ? [buildLiteral(value)] : [];

      return { flags, type, children, properties: {}, attributes };
